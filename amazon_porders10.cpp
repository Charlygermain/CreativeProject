...
ifstream in;
in.open("purchase_data_Assignment10.txt");
if(in.is_open())
{
while (!in.eof())
{
...
cout<<"Cannot open the file purchase_data.txt";
in.close();
}
/***********************************************************************************************************************************/
//Name: is_Empty
...
//Precondition:
//Postcondition:
//Description: adds a call_record to INV; if INV is full, double_size is called to increase the size of INV. The user
...
int i = search(key);
if(i == -1)
cout<<"No record with this key : "<<key<<" exists"<<endl;
else{
while(i != -1)
{
